...
//1个16位数拆分为2个8位数
//DataL为低位，DataH为高位
void LYS::Host2SCS(u8 *DataL, u8* DataH, u16 Data)
{
	if(End){
		*DataL = (Data>>8);
		*DataH = (Data&0xff);
...
//2个8位数组合为1个16位数
//DataL为低位，DataH为高位
u16 LYS::SCS2Host(u8 DataL, u8 DataH)
{
	u16 Data;
	if(End){
		Data = DataL;
...
void LYS::writeBuf(u8 ID, u8 MemAddr, u8 *nDat, u8 nLen, u8 Fun)
{
	u8 msgLen = 2;
	u8 bBuf[6];
	u8 CheckSum = 0;
...
int LYS::genWrite(u8 ID, u8 MemAddr, u8 *nDat, u8 nLen)
{
	rFlushSCS();
	writeBuf(ID, MemAddr, nDat, nLen, LYST_WRITE);
	wFlushSCS();
	return Ack(ID);
}
...
//异步写指令
//舵机ID，MemAddr内存表地址，写入数据，写入长度
int LYS::regWrite(u8 ID, u8 MemAddr, u8 *nDat, u8 nLen)
{
	rFlushSCS();
	writeBuf(ID, MemAddr, nDat, nLen, LYST_REG_WRITE);
	wFlushSCS();
	return Ack(ID);
}
...
//异步写执行指令
//舵机ID
int LYS::RegWriteAction(u8 ID)
{
	rFlushSCS();
	writeBuf(ID, 0, NULL, 0, LYST_REG_ACTION);
	wFlushSCS();
	return Ack(ID);
}
...
void LYS::syncWrite(u8 ID[], u8 IDN, u8 MemAddr, u8 *nDat, u8 nLen)
{
	rFlushSCS();
	...
	bBuf[2] = 0xfe;
	bBuf[3] = mesLen;
	bBuf[4] = LYST_SYNC_WRITE;
	bBuf[5] = MemAddr;
	bBuf[6] = nLen;
	writeSCS(bBuf, 7);

	Sum = 0xfe + mesLen + LYST_SYNC_WRITE + MemAddr + nLen;
	u8 i, j;
	for(i=0; i<IDN; i++){
		writeSCS(ID[i]);
...
int LYS::writeByte(u8 ID, u8 MemAddr, u8 bDat)
{
	rFlushSCS();
	writeBuf(ID, MemAddr, &bDat, 1, LYST_WRITE);
	wFlushSCS();
	return Ack(ID);
}

int LYS::writeWord(u8 ID, u8 MemAddr, u16 wDat)
{
	u8 bBuf[2];
	Host2SCS(bBuf+0, bBuf+1, wDat);
	rFlushSCS();
	writeBuf(ID, MemAddr, bBuf, 2, LYST_WRITE);
	wFlushSCS();
	return Ack(ID);
}
...
int LYS::Read(u8 ID, u8 MemAddr, u8 *nData, u8 nLen)
{
	rFlushSCS();
	writeBuf(ID, MemAddr, &nLen, 1, LYST_READ);
	wFlushSCS();
	u8Error = ERR_NORMAL;
	if(!checkHead()){
...
//读1字节，超时返回-1
int LYS::readByte(u8 ID, u8 MemAddr)
{
	u8 bDat;
...
//读2字节，超时返回-1
int LYS::readWord(u8 ID, u8 MemAddr)
{	
	u8 nDat[2];
	int Size;
	u16 wDat;
...
{
	rFlushSCS();
	writeBuf(ID, 0, NULL, 0, LYST_PING);
	wFlushSCS();
	u8Status = 0;
	if(!checkHead()){
		u8Error = ERR_NO_REPLY;
...
int	LYS::syncReadPacketTx(u8 ID[], u8 IDN, u8 MemAddr, u8 nLen)
{
	rFlushSCS();
	syncReadRxPacketLen = nLen;
	u8 checkSum = (4+0xfe) + IDN + MemAddr + nLen + LYST_SYNC_READ;
	u8 i;
	writeSCS(0xff);
	writeSCS(0xff);
	writeSCS(0xfe);
	writeSCS(IDN+4);
	writeSCS(LYST_SYNC_READ);
	writeSCS(MemAddr);
	writeSCS(nLen);
	for(i=0; i<IDN; i++){
		writeSCS(ID[i]);
...
void LYS::syncReadBegin(u8 IDN, u8 rxLen, u32 TimeOut)
{
...
int LYS::syncReadPacketRx(u8 ID, u8 *nDat)
{
...
int LYS::syncReadRxPacketToByte()
{
...
int LYS::syncReadRxPacketToWrod(u8 negBit)
{
...
{
	rFlushSCS();
	writeBuf(ID, 0, NULL, 0, LYST_RESET);
	wFlushSCS();
	return Ack(ID);
}
